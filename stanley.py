...
# startDate = "2015-06-01T07:00:00"
# endDate = "2015-09-21T07:00:00"
#endDate = "2015-06-08T07:00:00"
...
#         plt.show()
        ...
        plt.ylabel('Reheat Discharge (PSI)')
...
def runSegmentationForZone(t, x1, plotFlag, USE_ALL=False, sagFlag=True, bw=0):
    ...
    intervals1 = edgeDetector( [x1[training_idx] for training_idx in training_set], [6], [.2], bw, plotFlag, sagFlag)
...
results = smap_client.query("select distinct Metadata/roomid where Metadata/SourceName='StanleyHallBMSNew'")
...
for room in results:
    try:
        ...
        if room == "all":
            continue
        ret = smap_client.query("select uuid where Metadata/roomid='%s' and Metadata/sensor='room_temp'" % room)
        if len(ret) == 0:
            continue
        temp_uuid = str(ret[0]["uuid"])
        ret = smap_client.query("select uuid where Metadata/roomid='%s' and Metadata/sensor='supply_air_velocity'" %room)
        if len(ret) == 0:
            continue
        flow_uuid = str(ret[0]["uuid"])
        ret = smap_client.query("select uuid where Metadata/roomid='%s' and Metadata/sensor='reheat_valve_position'" % room)
        if len(ret) == 0:
            continue
        reheat_uuid = str(ret[0]["uuid"])
        ret = smap_client.query("select Metadata/ahu where Metadata/roomid='%s'and Metadata/sensor='reheat_valve_position'" % room)
        if len(ret) == 0:
            continue
        ahuid = str(ret[0]["Metadata"]["ahu"])
        ret = smap_client.query("select uuid where Metadata/ahu='%s' and Metadata/sensortype='supply_air_temp'" % ahuid)
        if len(ret) == 0:
            continue
        sat_uuid = str(ret[0]["uuid"])
        room_list[room] = {}
        room_list[room]["temp"] = temp_uuid
        room_list[room]["flow"] = flow_uuid
        room_list[room]["reheat"] = reheat_uuid
        room_list[room]["sat"] = sat_uuid
        room_list[room]["ahu"] = ahuid
...
#         ax[4].set_ylabel('ts < 16hr', size = 'x-large')
        ax[3].set_xlim(0.0, len(imfs[0]))
        numdays = len(imfs[0])/numperiods
        labels = day_labels*(numdays/7)+day_labels[0:(numdays%7)]
        ax[3].set_xticks(np.arange(0, len(imfs[0]), numperiods))
...
def plotOccupancy(weekly_occupancy_bitmap, number_of_weeks, zone):
    ...
    plt.title('Stanley ' + str(zone) + ' between '+str(startDate_dt_local)+' and '+str(endDate_dt_local))
    plt.show()
...
pylab.rcParams['figure.figsize'] = (20.0, 70.0)
...
#     plt.title("Average Energy Loss [Stanley Hall]", y=1.05)
#     for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] + ax.get_xticklabels() + ax.get_yticklabels()):
#         item.set_fontsize(20)
#     plt.show()
...
#     plt.colorbar(heatmap)
    ...
    ax.set_title("Average Occupancy [Stanley Hall]", y=1.005)
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] + ax.get_xticklabels() + ax.get_yticklabels()):
        item.set_fontsize(20)
    plt.show()
    ...
    try:
        for i in range(len(avg_weekly_occupancy_bitmap[zone])):
            if avg_weekly_occupancy_bitmap[zone][i] >= 0.75*max(avg_weekly_occupancy_bitmap[zone]):
                count_occ += 1
            elif avg_weekly_occupancy_bitmap[zone][i] <= 1.25*min(avg_weekly_occupancy_bitmap[zone]):
                count_unocc += 1
                
        if count_occ > 0.75 * len(avg_weekly_occupancy_bitmap[zone]):
            always_occupied.append(zone)
        if count_unocc > 0.75 * len(avg_weekly_occupancy_bitmap[zone]):
...
north_facing = ['274A', '260K', '260J', '260H', '260G', '260F', '260E', '260D', '260C', '256', '250', '350', '356', '360', '370', '374A',                 '450', '456', '460', '550', '556', '650']

northwest_facing = ['750', '706A', '606A', '650', '508A', '556', '374B', '274B', '174D']
northeast_facing = []

south_facing = ['712', '706D', '606D', '612', '512', '506D', '412', '406D', '312', '300A', '306D', '306E', '306F', '306G',                 '212', '208D', '200A', '206']

southwest_facing = ['808', '708C', '608C', '508C', '408C', '308C', '306H', '206C', '208C', '108A', '104A']
southeast_facing = ['621', '521', '421', '306C', '321', '206D']

east_facing = ['725', '731A', '731', '751', '625', '631A', '631', '651', '527', '531', '551', '424', '430', '432',                '451', '324C', '324D', '324E', '324', '324F', '324G', '324H', '330A', '330', '351']

west_facing = ['708B', '708A', '708', '742', '748', '740', '608A', '608B', '608', '642', '648', '640', '508A',                '508B', '508', '542', '592B', '576', '574', '572', '570', '408A', '408B', '478', '476', '474', '472', '470',                '308B', '308A', '306J', '306K', '304A', '304B', '378J', '378H', '378G', '378F', '378E', '378D', '378C', '374D',                '374C', '208A', '206B', '204A', '204B', '202', '278', '274D', '274C', '108', '104A', '104', '178', '174B', '174C', '174F']

middle = []
for zone in normal_zones:
    if zone not in north_facing+northeast_facing+east_facing+southeast_facing+south_facing+southwest_facing+west_facing+northwest_facing:
        middle.append(zone)
...
def plotSortedHeatMap(zones):
    pylab.rcParams['figure.figsize'] = (20.0, 55.0)
    ...
    such_zones = []
    for zone in zones:
        if zone in sorted(normal_zones, key=lambda k: zone):
...
#     plt.colorbar(heatmap)
    ...
    ax.set_xticklabels(col_labels)
    ax.set_yticklabels(row_labels)
    ax.set_title("Apparent Occupancy Heat Map", y=1.01)
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] + ax.get_xticklabels() + ax.get_yticklabels()):
        item.set_fontsize(20)
    plt.show()
...
# pylab.rcParams['figure.figsize'] = (40.0, 10.0)
# plt.figure()
# bar_width = 0.35
